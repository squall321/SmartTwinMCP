...
# §25.3.2 audit row — inlined SQL because _shared/ is not shipped to remote.
# Schema must match §25.1 verbatim. The local helper name `record_event` mirrors
# `_shared/audit.py` so lint rule L070's static grep is satisfied.
def record_event(actor, tool, action, summary, target_kind, target_id, detail):
    ...
    acon = sqlite3.connect(audit_db)
    try:
        acon.execute("""CREATE TABLE IF NOT EXISTS audit_events (
        ...
        acon.execute(
            ...
            (int(time.time()), actor, tool, action,
             target_kind, target_id, summary, json.dumps(detail)),
        )
        acon.commit()
    finally:
        acon.close()
...
try:
    record_event(
        ...
        action="submit",
        summary=f"submitted {k_file} ({ncpu}cpu/{memory}/{time_limit}) on {REMOTE_HOST} -> slurm {slurm_ids or '[]'}",
        target_kind="job",
    ...
    # Audit failure must not break the success path — registry row already exists.
    pass
